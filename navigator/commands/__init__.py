    ...
    def handle(self, **kwargs):
        ...
        try:
            ...
            if not hasattr(self, self.action):
                self.write('Error: Method **{}** not found on {}'.format(self.action, str(self)), level='ERROR')
                raise CommandNotFound('Method {} from {} not Found'.format(self, self.action))
    ...
    if len(sys.argv) > 1:
        ...
        if command is not None:
            ...
            if program in INSTALLED_APPS:
                ...
                classpath = '{program}.commands.{provider}'.format(program=program, provider=clsCommand)
                try:
                    module = importlib.import_module(classpath, package=clsCommand)
                    cls = getattr(module, clsCommand)
                except ImportError:
                    raise CommandNotFound("Command %s for program %s was not found" % (clsCommand, program))
            else:
                ...
                try:
                    ...
                    raise CommandNotFound("Command %s was not found o program doesnt exists" % clsCommand)
            try:
                cmd = cls(args)
