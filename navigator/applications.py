        ...
        for route in list(self.app.router.routes()):
            ...
            if doc is None:
                ...
                if signature.return_annotation:
                    ...
                    response = 'unknown'
                if fnName not in ['_handle', 'channel_handler', 'WebSocket', "websocket"]:
                    doc = """
                    summary: {fnName}
                    produces:
                    - text/plain
...
class AppConfig(AppHandler):
    ...
    async def on_startup(self, app):
        ...
        pool = AsyncPool('pg', dsn=app['config']['asyncpg_url'], loop=self._loop, timeout=360000, **kwargs)
        await pool.connect()
    ...
    def setup_routes(self):
        ...
        try:
            cls = importlib.import_module('{}.{}'.format('apps.{}'.format(self._name), 'urls'), package='apps')
        ...
        for route in routes:
            if inspect.isclass(route.handler) and issubclass(route.handler, AbstractView):
                if not route.method:
                    ...
                    r = self.app.router.add_route("*", route.url, route.handler, name=route.name)
                ...
                if not route.method:
                    ...
                    if route.method == 'get':
                        r = self.app.router.add_get(route.url, route.handler, name=route.name)
                    elif route.method == 'post':
                        r = self.app.router.add_post(route.url, route.handler, name=route.name)
                    elif route.method == 'delete':
                        r = self.app.router.add_delete(route.url, route.handler, name=route.name)
                    elif route.method == 'patch':
                        r = self.app.router.add_patch(route.url, route.handler, name=route.name)
                    else:
                        raise('Unsupported Method for Route {}, program: {}'.format(route.method, self._name))
                        return False
                    self.cors.add(r)
