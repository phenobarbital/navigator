...
# aiohttp session
from aiohttp_session import get_session, setup as setup_session, new_session
# Storages
...
class AuthHandler(AbstractHandler):
    ...
    async def login(self, request) -> web.Response:
        ...
        if await self.check_credentials(login, password):
            ...
            template = self._template.format(message='Invalid username/password combination')
