...
from aiohttp.web_exceptions import HTTPNoContent, HTTPMethodNotAllowed, HTTPClientError, HTTPUnauthorized, HTTPNotImplemented, HTTPInternalServerError
...
class BaseHandler(CorsViewMixin):
    ...
    def not_implemented(self, request: web.Request, response: dict={}, headers: dict={}, **kwargs) -> web.Response:
        args = {
        ...
        try:
            ...
            if not args or len(args) == 0:
                if self.request.body_exists:
                    ...
                    if body:
                        try:
                            args = dict( (k, v if len(v)>1 else v[0] ) for k, v in parse.parse_qs(body).items() )
